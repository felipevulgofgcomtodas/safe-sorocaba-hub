...
import { Shield, Mail, Lock, User, MapPin, ArrowLeft, Loader2 } from "lucide-react";
...
const Cadastro = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    setError("");
    const ok = await signup(name, email, password, address);
    setLoading(false);
    if (ok) navigate("/");
    ...
    <div className="min-h-screen bg-background mesh-gradient flex items-center justify-center px-4 py-12">
      <div className="w-full max-w-md animate-in fade-in zoom-in duration-500">
        ...
        <div className="glass-strong rounded-3xl p-8 border border-white/5 shadow-2xl">
          ...
          {error && (
            <div className="mb-4 p-3 rounded-lg bg-destructive/10 border border-destructive/20 text-sm text-destructive animate-in fade-in">
              {error}
            </div>
          )}
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            {fields.map((f) => (
              <div key={f.label} className="space-y-1.5">
                <label className="text-sm font-semibold text-foreground/90 ml-1">{f.label}</label>
                <div className="flex items-center gap-3 px-4 py-3.5 rounded-xl bg-muted/30 border border-white/5 focus-within:border-primary/50 focus-within:ring-4 focus-within:ring-primary/10 transition-all">
            ...
            <button type="submit" disabled={loading} className="w-full py-3.5 rounded-xl bg-primary text-primary-foreground font-bold text-sm hover:bg-primary/90 transition-all disabled:opacity-50 active:scale-[0.98] shadow-lg shadow-primary/20 flex items-center justify-center gap-2 mt-2">
          ...
          <div className="flex items-center gap-4 my-7">
            ...
            <span className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground/60">Ou</span>
            <div className="flex-1 h-px bg-white/5" />
          </div>

          <button onClick={handleGoogle} disabled={loading} className="w-full flex items-center justify-center gap-3 py-3 rounded-xl border border-white/10 text-sm font-medium text-foreground hover:bg-white/5 transition-all active:scale-[0.97] disabled:opacity-50">
